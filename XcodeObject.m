...
@implementation XcodeObject @synthesize icon = _icon, humanUTI = _humanUTI; static NSArray *iconMap = nil;

+ (instancetype) objectWithName:(NSString*)n uuid:(NSString*)uuid {
	XcodeObject *x = self.new;	x.name = n;	x.uuid = uuid;	x.children = @[].mutableCopy; 	return x;   //	x.parent = NSNull.null;
}
-(id) mutableCopyWithZone: (NSZone *)z	{
        XcodeObject *cp = [XcodeObject allocWithZone:z];
			cp.name = _name;	cp.uuid = _uuid;	//cp.parent = _parent; // if (_catchall.count) cp.catchall = _catchall;
			cp.children = _children ?: @[].mutableCopy;
        return cp;
}
+ (NSSet*) keyPathsForValuesAffectingIcon 							{ return [NSSet setWithArray:@[@"name"]]; }
- (void) setValue:(id)v forUndefinedKey:	 		(NSString*)k 	{	NSLog(@"need to set V for udk:%@", k); }
-   (id) valueForUndefinedKey:				      (NSString*)k 	{ return  nil; }
- (void) setObject:(id)x forKeyedSubscript:(id <NSCopying>)k 	{

	if ([self respondsToSelector:NSSelectorFromString((NSString*)k)]) [self setValue:x forKey:(NSString*)k];
}
- (id)objectForKeyedSubscript:(id)k { return [self valueForKey:(NSString*)k]; }

- (NSImage*) icon {  if (!_name) return [NSImage imageNamed:NSImageNameRefreshTemplate]; NSString *ext = _name.pathExtension;
...
- (NSString*) humanUTI {  return  _humanUTI = _humanUTI ?: humanReadableFileTypeForFileExtension(self.name.pathExtension) ?: @"Folder"; }
